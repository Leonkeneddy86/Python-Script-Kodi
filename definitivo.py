...
CHANNEL_ID_2 = -100000000  # Ejemplo de ID para el segundo canal
CONTROL_CHANNEL_ID = -100200000  # ID del canal de control
...
async def process_grouped_files(client, message, save_dir, extract_dir):
    ...
    if is_archive:
        if archive_type in ['7z', 'zip']:
            ...
            files_processed = await extract_file(joined_file_path, extract_dir)
        elif archive_type == 'rar':
            files_processed = await extract_file(rar_main_file, extract_dir)
...
async def process_channel(client, channel_id, save_dir, extract_dir, last_message_id, tmm_command):
    try:
        ...
        async for message in client.iter_messages(channel, min_id=last_message_id):
            ...
            files_processed = files_processed or processed
