...
# IDs de los canales
CHANNEL_ID_1 = -10000000  # Canal 1 (Movies)
CHANNEL_ID_2 = -100200000 # Canal 2 (Series)
CONTROL_CHANNEL_ID = -100200000  # Canal de control
...
def execute_tmm(command):
    """Ejecutar TinyMediaManager."""
    try:
        subprocess.run(command.split(), check=True)  # Cambiado a split() para evitar problemas con espacios en la ruta.
        print(f"TinyMediaManager ejecutado: {command}")

    except subprocess.CalledProcessError as e:
        print(f"Error al ejecutar TinyMediaManager: {e}")
...
async def process_channel(client, channel_id, save_dir, extract_dir, last_message_id):
    try:
        ...
        # Ejecutar TinyMediaManager si se procesaron archivos
        if files_processed and USE_TMM:
            command_to_execute = TMM_CHANNEL_ID_1_COMMAND if channel_id == CHANNEL_ID_1 else TMM_CHANNEL_ID_2_COMMAND
            execute_tmm(command_to_execute)
